...
WHEN sub_status IN (${ISSUE_SUB_STATUS_NUM.SITE_VISIT_ASSIGNED}, ${ISSUE_SUB_STATUS_NUM.WORK_ASSIGNED}) AND info IS NOT NULL AND JSON_VALID(info) AND JSON_CONTAINS_PATH(info, 'one', '$.agent_ids') THEN 
  ...
  getIssueHistoryForCustomer(database) {
    ...
    END as visitTime,
    CASE 
      WHEN time_slot = ${TIME_SLOTS_NUM.MORNING} THEN '${TIME_SLOTS_STRING.MORNING}'
      WHEN time_slot = ${TIME_SLOTS_NUM.AFTERNOON} THEN '${TIME_SLOTS_STRING.AFTERNOON}'
      WHEN time_slot = ${TIME_SLOTS_NUM.NONE} THEN '${TIME_SLOTS_STRING.NONE}'
    END as time_slot,
    CASE
